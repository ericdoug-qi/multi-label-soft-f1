...
class MovieInput(ImageNetInput):
    """Movielens dataset"""
    cfg = copy.deepcopy(ImageNetInput.cfg)
    cfg.update(
        dict(
            ...
            tfds_name="movies",
            splits=dict(
                train=dict(num_images=29561, tfds_split='train', slice=''),
                minival=dict(num_images=7391, tfds_split='test', slice=''),
                eval=dict(num_images=7391, tfds_split='test', slice=''),

            )
        )
    )

    def preprocess(self, features, mlb):
        ...
        new_label = {'label': mlb.transform(features['label'])}
    ...
    def parse_function(self, filename, label):
        ...
        image_resized = tf.image.resize(image_decoded, [self.image_size, 224])
        # Normalize it from [0, 255] to [0.0, 1.0]
        image_normalized = image_resized / 255.0
        # return image_normalized, label
        new_features = {'image': image_normalized}
    ...
    def create_dataset(self, filenames, labels, batch_size, current_host, num_hosts, is_training=True):
        ...
        if is_training == True:
            if self.cache:
                ...
                dataset = dataset.shuffle(buffer_size=16 * 1024, seed=self.shuffle_seed)
            else:
                dataset = dataset.shuffle(self.shuffle_size_k * 1024, seed=self.shuffle_seed)
    ...
    def _input_fn(self, batch_size, current_host, num_hosts):
        ...
        return self.create_dataset(X, y_bin, batch_size, current_host, num_hosts)
...
@ds_register
class MoviesFt(ImagenetFt):
    """Finetune cifar10 configs."""
    cfg = copy.deepcopy(ImagenetFt.cfg)
    cfg.data.ds_name = 'movies'
